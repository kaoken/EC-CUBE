...
class plg_ProductImagesAddKN_Util
{
	 ...
	 * @return bool
	 */
	public function notModifiedHeaders($lastModifiedTime, $seedEtag)
	{
		$etag = md5($seedEtag);
	 ...
	 * @return object|plg_ProductImagesAddKN_DB_Base
	 */
	...
	{
		$dir = $isCashImgDir?$this->getHtmlDirPath('/cash_images'):$this->getHtmlDirPath('/temp_images');
	...
	{
		return $this->getHtmlDirPath('/temp_images').$fileName;
	}
	//###########################################################
	//##
	//## ファイル処理関係
	...
	{
		$file = new stdClass();
		$file->path = $this->getPHtmlPath('/temp', $name);
